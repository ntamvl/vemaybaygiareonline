...
if(!class_exists('STActivity'))
{
    class STActivity extends TravelerObject
    {
        ...
        function init()
        {
            ...
            // add to cart
            add_action('init',array($this,'activity_add_to_cart'));
        ...
        function get_result_string()
        {
            ...
            if($wp_query->found_posts > 1){
                $result_string.=esc_html( $wp_query->found_posts).__(' activities ',ST_TEXTDOMAIN);
            }else{
                $result_string.=esc_html( $wp_query->found_posts).__(' activity ',ST_TEXTDOMAIN);
            }
            ...
            if($location_id and $location=get_post($location_id))
            {
                ...
                if(!empty($_REQUEST['s'])){
                    $result_string.='in '.STInput::get('s');
                }
